...
async function extractFromImage(base64Image, apiKey) {
  const systemPrompt = [
    "You are an expert OCR system reading a printed DELIVERY SLIP form filled in by hand.",
    "",
    ...
    "",
    "Each label is printed in small capital letters. Below each label is a line where the person wrote their answer in pen.",
    "The handwriting may be cursive, print, or mixed. Pen colour may be blue, black or green.",
    "Even if you are only 60% sure, give your best reading - never say illegible.",
    "South African vehicle registration format: 2 letters + digits e.g. CY 236 861 or CA 123-456.",
    "Dates may be written as: 29 MAY 26 or 29/05/26 or similar.",
    "Times may be written as: 2:49 or 14:30 or 08h00.",
    "Only return empty string if the field has absolutely no writing.",
    "",
  ...
  const response = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      messages: [{ role: "user", content: [
        ...
        { type: "text", text: "Read every handwritten field on this delivery slip. Return only the JSON." }
      ]}]
    })
  });
...
function SlipTemplate({ captureUrl }) {
  const fields = ["DATE","TIME IN","SUPPLIER / COMPANY NAME","DRIVER NAME","VEHICLE REGISTRATION","DELIVERY NOTE / PO NO."];
  return (
    <div style={{ fontFamily: "'Courier New', monospace", background: "#fff", color: "#111", width: 400, padding: "20px 24px", border: "2px solid #111", borderRadius: 4, boxSizing: "border-box" }}>
      <div style={{ textAlign: "center", borderBottom: "2px solid #111", paddingBottom: 10, marginBottom: 12 }}>
        <div style={{ fontSize: 10, letterSpacing: 4, textTransform: "uppercase", color: "#555", marginBottom: 4, fontWeight: 700 }}>Supplier Delivery Register</div>
        <div style={{ fontSize: 20, fontWeight: 900, letterSpacing: 2, textTransform: "uppercase" }}>DELIVERY SLIP</div>
        <div style={{ fontSize: 10, color: "#888", marginTop: 4, fontWeight: 700 }}>Complete all fields clearly in BLOCK LETTERS</div>
      </div>
      {fields.map(label => (
        <div key={label} style={{ marginBottom: 12 }}>
          <div style={{ fontSize: 10, letterSpacing: 1, textTransform: "uppercase", color: "#555", marginBottom: 3, fontWeight: 900 }}>{label}</div>
          <div style={{ borderBottom: "1.5px solid #333", height: 22 }} />
        </div>
      ))}
      <div style={{ marginTop: 6, marginBottom: 14 }}>
        <div style={{ fontSize: 10, letterSpacing: 1, textTransform: "uppercase", color: "#555", marginBottom: 3, fontWeight: 900 }}>Driver Signature</div>
        <div style={{ borderBottom: "1.5px solid #333", height: 26 }} />
      </div>
      <div style={{ borderTop: "2px dashed #aaa", paddingTop: 12, display: "flex", alignItems: "center", gap: 14 }}>
        <QRCode url={captureUrl} size={72} />
        <div>
          <div style={{ fontSize: 9, fontWeight: 900, letterSpacing: 1, textTransform: "uppercase", marginBottom: 4 }}>Scan to Register Delivery</div>
          <div style={{ fontSize: 8, color: "#555", lineHeight: 1.6, fontWeight: 700 }}>Gate / warehouse staff must scan<br />this code and photograph the slip<br />to complete the delivery record.</div>
        </div>
      </div>
    </div>
  );
...
function CapturePage({ onSave, onBack, apiKey }) {
  ...
  const handleExtract = async () => {
    ...
    setError("");
    try {
      const data = await extractFromImage(imageBase64, apiKey);
      setEditFields(data);
      setStep("review");
    } catch (e) {
      setError("Could not read slip: " + e.message);
      setStep("capture");
    }
  };
  ...
  const handleSave = async () => {
    ...
    setUploadedAt(ts);
    await onSave({ id: uid(), timestamp: new Date().toISOString(), uploadedAt: ts, imagePreview, ...editFields });
    setStep("done");
  };

  const BackBtn = ({ label, action }) => (
    <button onClick={action} style={{ background: "transparent", border: "1px solid #1e3a5f", borderRadius: 6, color: "#8892a4", fontSize: 13, fontFamily: "'Courier New', monospace", padding: "8px 16px", cursor: "pointer", letterSpacing: 1, fontWeight: 700, marginBottom: 20 }}>
      {label}
    </button>
  );

  const spinner = (
    <div style={{ minHeight: "100dvh", background: "#0d1117", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ color: "#8892a4", fontSize: 14, fontFamily: "'Courier New', monospace", fontWeight: 700 }}>
        {step === "extracting" ? "Reading handwriting..." : "Saving to register..."}
      </div>
    </div>
  );

  if (step === "extracting" || step === "saving") return spinner;
  ...
  if (step === "done") return (
    <div style={{ minHeight: "100dvh", background: "#0d1117", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24 }}>
      ...
      <div style={{ color: "#fff", fontSize: 22, fontWeight: 900, fontFamily: "'Courier New', monospace", letterSpacing: 2, marginBottom: 8 }}>LOGGED</div>
      <div style={{ color: "#8892a4", fontSize: 13, textAlign: "center", marginBottom: 6, fontWeight: 700 }}>Delivery has been recorded in the register.</div>
      <div style={{ color: "#4f8ef7", fontSize: 13, fontFamily: "'Courier New', monospace", fontWeight: 900, marginBottom: 28 }}>📅 {uploadedAt}</div>
      <button onClick={onBack} style={{ background: "#1e3a5f", border: "none", borderRadius: 6, color: "#fff", fontSize: 13, fontFamily: "'Courier New', monospace", padding: "10px 20px", cursor: "pointer", fontWeight: 700 }}>← Back to Home</button>
    </div>
  );

  if (step === "review") return (
    <div style={{ minHeight: "100dvh", background: "#0d1117", padding: "20px 16px 40px", fontFamily: "'Courier New', monospace" }}>
      <BackBtn label="← Back" action={() => setStep("capture")} />
      ...
      <div style={{ color: "#8892a4", fontSize: 13, marginBottom: 20, fontWeight: 700 }}>Check extracted fields. Correct anything wrong.</div>
      ...
      {FIELDS.map(({ key, label }) => (
        <div key={key} style={{ marginBottom: 14 }}>
          <div style={{ color: "#4f8ef7", fontSize: 11, letterSpacing: 2, textTransform: "uppercase", marginBottom: 4, fontWeight: 900 }}>{label}</div>
          <input value={editFields[key] || ""} onChange={e => setEditFields(prev => ({ ...prev, [key]: e.target.value }))}
            style={{ width: "100%", boxSizing: "border-box", background: "#131b27", border: "1px solid #1e3a5f", borderRadius: 6, padding: "12px 14px", color: "#fff", fontSize: 17, fontFamily: "'Courier New', monospace", outline: "none", fontWeight: 700 }} />
        </div>
      ))}
  ...
  return (
    <div style={{ minHeight: "100dvh", background: "#0d1117", padding: "20px 20px 40px", fontFamily: "'Courier New', monospace", display: "flex", flexDirection: "column", alignItems: "center" }}>
      <div style={{ width: "100%", maxWidth: 340 }}>
        <BackBtn label="← Back" action={onBack} />
      </div>
      ...
      <div style={{ color: "#8892a4", fontSize: 13, textAlign: "center", marginBottom: 24, maxWidth: 280, fontWeight: 700 }}>
        Hold phone flat above the slip. Make sure all fields are visible and well lit.
      </div>
      ...
      <div onClick={() => cameraRef.current.click()}
        ...
        {imagePreview
          ...
          : <><div style={{ fontSize: 48, marginBottom: 12 }}>📷</div><div style={{ color: "#4f8ef7", fontSize: 13, letterSpacing: 1, fontWeight: 700 }}>Tap to take photo</div></>
        }
      </div>
      ...
      <button onClick={() => galleryRef.current.click()}
        style={{ width: "100%", maxWidth: 340, padding: "12px", background: "transparent", border: "1px solid #1e3a5f", borderRadius: 8, color: "#8892a4", fontSize: 13, fontFamily: "'Courier New', monospace", letterSpacing: 2, textTransform: "uppercase", cursor: "pointer", marginBottom: 16, fontWeight: 700 }}>
        🖼 &nbsp;CHOOSE FROM GALLERY
      </button>

      {error && <div style={{ color: "#ff5252", fontSize: 13, marginBottom: 12, fontWeight: 700, textAlign: "center", maxWidth: 340 }}>{error}</div>}
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#070b12", fontFamily: "'Courier New', monospace", color: "#fff" }}>
      <div style={{ padding: "24px 24px 20px", borderBottom: "1px solid #1a2540" }}>
        ...
        <div style={{ color: "#8892a4", fontSize: 12, marginTop: 4, fontWeight: 700 }}>{entries.length} record{entries.length !== 1 ? "s" : ""} logged</div>
      </div>
      {entries.length === 0
        ? <div style={{ padding: 40, textAlign: "center", color: "#8892a4", fontSize: 13, fontWeight: 700 }}>No deliveries recorded yet.</div>
        : <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              <thead>
                <tr style={{ background: "#0d1117" }}>
                  <th style={{ padding: "10px 12px", textAlign: "left", color: "#4f8ef7", fontSize: 9, letterSpacing: 2, textTransform: "uppercase", whiteSpace: "nowrap", fontWeight: 900 }}>#</th>
                  {FIELDS.map(f => <th key={f.key} style={{ padding: "10px 12px", textAlign: "left", color: "#4f8ef7", fontSize: 9, letterSpacing: 2, textTransform: "uppercase", whiteSpace: "nowrap", fontWeight: 900 }}>{f.label}</th>)}
                  <th style={{ padding: "10px 12px", color: "#4f8ef7", fontSize: 9, letterSpacing: 2, textTransform: "uppercase", fontWeight: 900 }}>IMG</th>
                </tr>
              </thead>
              <tbody>
                {[...entries].reverse().map((e, i) => (
                  <tr key={e.id} style={{ borderBottom: "1px solid #1a2540", background: i % 2 === 0 ? "#0d1117" : "transparent" }}>
                    <td style={{ padding: "10px 12px", color: "#8892a4", fontSize: 11, fontWeight: 700 }}>{entries.length - i}</td>
                    {FIELDS.map(f => <td key={f.key} style={{ padding: "10px 12px", color: "#e2e8f0", whiteSpace: "nowrap", maxWidth: 160, overflow: "hidden", textOverflow: "ellipsis", fontWeight: 700 }}>{e[f.key] || <span style={{ color: "#3a4a60" }}>—</span>}</td>)}
...
export default function App() {
  ...
  if (!loaded) return (
    <div style={{ minHeight: "100vh", background: "#070b12", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ color: "#4f8ef7", fontFamily: "monospace", fontSize: 13, letterSpacing: 2, fontWeight: 700 }}>LOADING...</div>
    </div>
  );

  if (view === "capture") return <CapturePage onSave={handleSave} onBack={() => setView("home")} apiKey={apiKey} />;
    ...
    <div style={{ minHeight: "100vh", background: "#f0f0e8", display: "flex", flexDirection: "column", alignItems: "center", padding: "32px 16px" }}>
      ...
      <style>{`
        @media print {
          ...
          .print-page { width: 285mm; height: 197mm; display: flex; flex-direction: row; align-items: flex-start; justify-content: center; gap: 10mm; box-sizing: border-box; background: white; }
          .print-page > div { transform: scale(0.9); transform-origin: top left; }
        }
        @media screen {
      ...
      <div className="no-print" style={{ marginTop: 16, fontSize: 11, color: "#888", fontFamily: "monospace", textAlign: "center", maxWidth: 420, fontWeight: 700 }}>
        Prints 2 slips per A4 landscape page. Distribute to drivers.
      </div>
    </div>
  );
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#070b12", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 32, fontFamily: "'Courier New', monospace" }}>
      ...
      <div style={{ position: "relative", textAlign: "center", maxWidth: 380 }}>
        <div style={{ fontSize: 10, letterSpacing: 5, color: "#4f8ef7", textTransform: "uppercase", marginBottom: 12, fontWeight: 900 }}>Delivery Management System</div>
        ...
        <div style={{ marginTop: 40 }}>
          <div style={{ fontSize: 10, color: "#3a4a60", marginBottom: 12, letterSpacing: 2, fontWeight: 700 }}>QR CODE PREVIEW</div>
          <div style={{ display: "inline-block", background: "#fff", padding: 10, borderRadius: 8 }}>
            <QRCode url={CAPTURE_URL} size={120} />
          </div>
          <div style={{ fontSize: 9, color: "#3a4a60", marginTop: 8, letterSpacing: 1, fontWeight: 700 }}>This QR links to the capture form.<br />Print it on your slip template.</div>
        </div>
      </div>
    </div>
  );
...
const navBtn = { padding: "8px 16px", background: "#131b27", border: "1px solid #1e3a5f", borderRadius: 6, color: "#8892a4", fontSize: 12, fontFamily: "'Courier New', monospace", cursor: "pointer", letterSpacing: 1, fontWeight: 700 };
